...
Cout("String extraction engine build 20160131"&@CRLF&@CRLF)
...
If FileExists($datawin)==0 Then
	_Cmsg($CONMSG_CRITICAL,"CRITICAL: data.win doesn't exist in the specified path!"&@CRLF)
	Exit 10
EndIf
...
_Cmsg($CONMSG_INFO, "Trimming unneeded data..."&@CRLF&@CRLF)	;In order to speed up later tasks, we'll trim everything before STRG
$databin=StringTrimLeft($databin,$strgstart+7)
_Cmsg($CONMSG_INFO, "-----------------------"&@CRLF)
_Cmsg($CONMSG_INFO, "| STRG CHUNK METADATA |"&@CRLF)
_Cmsg($CONMSG_INFO, "-----------------------"&@CRLF)
;Now we read some data from STRG and save it for later use
Const $strgsize=ByteReverse(StringLeft($databin,8))	;Size of STRG
_Cmsg($CONMSG_INFO, "STRG Size: "&$strgsize&" (HEX)"&@CRLF)
$databin=StringTrimLeft($databin,8)
Const $strgnum=ByteReverse(StringLeft($databin,8))	;Number of strings in STRG
_Cmsg($CONMSG_INFO, "Num. of strings: "&$strgnum&" (HEX)"&@CRLF&@CRLF)
_Cmsg($CONMSG_INFO, "Writing metadata..."&@CRLF)
WriteMetadata()
_Cmsg($CONMSG_SUCCESS, "Metadata written to metadata.ini."&@CRLF)
WriteWarningFile()
_Cmsg($CONMSG_INFO, "Extracting strings - "&@CRLF)
$databin=StringTrimLeft($databin,8)
...
Do
	...
	_Console_SetCursorPosition(-1, 25, 19)
	Cout($percent&"%..."&@CRLF)										;Then overwrite what's there with the current percentage
	_ReduceMemory()
	$i+=1
Until $i==Dec($strgnum)
_Cmsg($CONMSG_SUCCESS, "Strings extracted!"&@CRLF)
